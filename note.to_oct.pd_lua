...
local note_to_oct = pd.Class:new():register('note.to_oct')

function note_to_oct:initialize(sel, atoms)
    self.inlets = 2
    self.outlets = 1
...
function note_to_oct:in_1_reload()
    self:dofilex(self._scriptname)
end

function note_to_oct:in_1_symbol(x)
    local note = mt.Note(x)
    note = note:to_octave(self.octave)
    self:outlet(1, "symbol", {tostring(note)})
end

function note_to_oct:in_2_float(x)
